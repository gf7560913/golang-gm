...
func ParseSm2PublicKey(der []byte) (*sm2.PublicKey, error) {
	...
	if !reflect.DeepEqual(pubkey.Algo.Algorithm, oidPublicKeySM2) {
...
func MarshalSm2PublicKey(key *sm2.PublicKey) ([]byte, error) {
	...
	algo.Algorithm = oidPublicKeySM2
...
func ParseSm2PrivateKey(der []byte) (*sm2.PrivateKey, error) {
	...
	if !reflect.DeepEqual(privKey.Algo.Algorithm, oidPublicKeySM2) {
		return nil, errors.New("x509: not sm2 elliptic curve")
	}
...
func ParseSm2EcryptedPrivateKey(der, pwd []byte) (*sm2.PrivateKey, error) {
	...
	rKey, err := ParseSm2PrivateKey(encryptedKey)
	if err != nil {
...
// MarshalSm2PrivateKey marshals an SM2 private key into ASN.1, DER format.
func MarshalSm2PrivateKey(key *sm2.PrivateKey) ([]byte, error) {
	_, ok := oidFromNamedCurve(key.Curve)
	if !ok {
		return nil, errors.New("x509: unknown elliptic curve")
	}

	return marshalSm2PrivateKey(key)
}
...
func MarshalSm2EcryptedPrivateKey(PrivKey *sm2.PrivateKey, pwd []byte) ([]byte, error) {
	der, err := marshalSm2PrivateKey(PrivKey)
	if err != nil {
		return nil, err
	}
	iter := 2048
