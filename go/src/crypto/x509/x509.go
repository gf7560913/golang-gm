...
func ParsePKIXPublicKey(derBytes []byte) (pub interface{}, err error) {
	...
	//add by xy
	algo := getPublicKeyAlgorithmFromOIDAndParam(pki.Algorithm.Algorithm, pki.Algorithm.Parameters.FullBytes)
...
//added by yj
func getPublicKeyAlgorithmFromOIDAndParam(oid asn1.ObjectIdentifier, fullBytes []byte) PublicKeyAlgorithm {
	switch {
	case oid.Equal(oidPublicKeyRSA):
		return RSA
	case oid.Equal(oidPublicKeyDSA):
		return DSA
	case oid.Equal(oidPublicKeyECDSA):
		sm2Bytes, _ := asn1.Marshal(oidNamedCurveP256SM2)
		switch {
		case bytes.Equal(fullBytes, sm2Bytes):
			return SM2
		default:
			return ECDSA
		}
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	case *sm2.PublicKey:
		...
		if !pub.Verify(signed, signature) {
			err = errors.New("x509: SM2 verification failure")
		}
		return
	}
	return ErrUnsupportedAlgorithm
...
func parseCertificate(in *certificate) (*Certificate, error) {
	...
	//added by yj
	fullBytes := in.TBSCertificate.PublicKey.Algorithm.Parameters.FullBytes
	out.PublicKeyAlgorithm =
		getPublicKeyAlgorithmFromOIDAndParam(in.TBSCertificate.PublicKey.Algorithm.Algorithm, fullBytes)
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	out := &CertificateRequest{
		...
		//modified by yj
		PublicKeyAlgorithm: getPublicKeyAlgorithmFromOIDAndParam(in.TBSCSR.PublicKey.Algorithm.Algorithm, in.TBSCSR.PublicKey.Algorithm.Parameters.FullBytes),
